...
class BrBeAttack(object):
    ...
    def generate(self,
                 ...
                 overshot=1.01,
                 targeted=False):
        save_path = 'data/BrBeAttack-Inf/'
        log = open(save_path+'log.txt', 'w')
        print("BrendelBethge-Inf Attack Information:", file=log)
        ...
        attack = BrendelBethgeAttack(estimator=classifier, norm=norm, overshoot=overshot, targeted=targeted)
...
if __name__ == "__main__":
    adv_generation = BrBeAttack()
    adv_generation.generate(norm=np.inf, targeted=False)
